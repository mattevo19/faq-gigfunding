...
export const activeHeader = (props) => {
  const propsDefault = {
    ...
    answers: ["Gigfunding is the change modern fundraising needs. Using our online marketplace, you can hire or volunteer skills depending on how you want to give; with the money paid for those skills going to the causes you want it to. Leaving behind passive direct debits, chugging and sponsored effing bungee jumps. Gigfunding allows you to fundraise when you want and how you want.",
      ...
      "As a UK registered non-profit organisation, we use a PAY-WHAT-YOU-FEEL revenue model and invite you to give a % of your payment for each transaction to Gigfunding; so we can continue to keep the platform running. You can choose to give us anything from 0-20%. This is an optional donation, not a fee, but we hope you can see the value in contributing to sustain the platform. Without donations, Gigfunding will not be able to continue and this is part of our philosophy of sharing the ownership of the project with YOU, the members.",
      "Currently the project is completely self-funded, and run by a team of like-minded and enthusiastic volunteers. In our first year (2017) we won £15,000 of grant funding, and have gratefully received £1645 through crowdfunding since. We are in the process of applying for private investment loans, to see us through our first 3 years of startup, to self-sustainability. Check out our Investment deck if you want to know the ins and outs.",
      "Use our online marketplace to hire or volunteer skills through the site. The skills for hire will cost market rates, and the money is donated to the causes the hirer and volunteer choose. See How it works for more detail.",
      ...
      "You can email us directly at team@gigfunding.org and we’ll get back to you as soon as possible. You can also use the live chat widget at the bottom left of your screen. We will endeavour to respond ASAP, but we’re only a small team of busy volunteers so please bare with us on response times!",
  ...
  if (!props.activeHeader) {
    return (
      <div className="active-header">
        <ul>
          {propsDefault.questions.map(question => <li className="question-list" key={question}>{question}</li>)}
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="active-header">
      <ul>
        {props.activeHeader.questions.map(question => <li className="question-list" key={question}>{question}</li>)}
      </ul>
    </div>
  );
};
